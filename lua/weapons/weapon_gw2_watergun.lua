...
function SWEP:PrimaryAttack()
	...
	gwater2.solver:AddCube(owner:EyePos() + forward * 20 * sprite_size * 39.3701, forward * gwater2["forward_velocity"], Vector(1, 1, 1) * gwater2["size"], sprite_size * gwater2["density"])
end
...
function SWEP:SecondaryAttack()
	...
	gwater2.solver:AddCube(owner:EyePos() + forward * 40 * sprite_size * 39.3701, forward * 500, Vector(33, 33, 33), sprite_size * 1)
end
...
hook.Add("PostDrawTranslucentRenderables", "gwater2_fuckthisshitman", function()
	...
	if IsValid(wep) and wep:GetClass() == "weapon_gw2_watergun" then
		...
		-- tiny cube
		local sprite_size = gwater2.solver:GetParameter("radius") * 39.3701
		local pos = owner:EyePos() + forward * 20 * sprite_size
		local size = Vector(1, 1, 1) * (gwater2["size"] * 0.5) * (sprite_size * gwater2["density"])

		-- big cube
		local sprite_size = gwater2.solver:GetParameter("fluid_rest_distance") * 39.3701
